...
const LevelView = ({ positions = [], currentBarber = null, services = [], apiRequest = null }) => {
  ...
  const latestHistory = levelProgress?.history?.[0] || null;
  ...
  const demPct = levelProgress?.demotionRisk ? Math.min(100, (levelProgress.demotionRisk.monthsFailed / levelProgress.demotionRisk.monthsThreshold) * 100) : 0;
    ...
    <div className="space-y-4">
      ...
      {activeTab === 'progress' && (
        <div className="space-y-4">
          ...
          {nextLevelPos && (
            <div className="relative overflow-hidden rounded-xl p-5" style={{ background: 'linear-gradient(135deg, var(--crm-surface-2), var(--crm-surface))', border: '1px solid rgba(0,191,175,0.12)' }}>
              <p className="text-[10px] uppercase tracking-widest text-[var(--crm-muted)] font-semibold mb-3">ТРЕБОВАНИЯ СЛЕДУЮЩЕГО УРОВНЯ: {nextLevelPos.name}</p>
