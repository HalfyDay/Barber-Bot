...
const fetchRemoteLicenses = async () => {
  try {
    ...
    console.warn('Не удалось загрузить список лицензий с GitHub:', error.message);
...
const validateLicenseRecord = (record, key) => {
  ...
  if (record.expiresAt && new Date(record.expiresAt).getTime() < Date.now()) {
    throw new Error('Срок действия лицензии истек');
  }
  return record;
};
...
const ensureLicenseValid = async (force = false) => {
  ...
  const isCacheFresh =
    cachedStatus.lastChecked &&
    Date.now() - new Date(cachedStatus.lastChecked).getTime() <
      LICENSE_REFRESH_SECONDS * 1000;
...
const licenseMiddleware = async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(403)
      .json({ error: 'Лицензия недействительна', details: error.message, status: getLicenseStatus() });
  }
};

const startLicenseWatcher = () => {
  setInterval(() => {
    ensureLicenseValid().catch((error) =>
      console.warn('Проверка лицензии не удалась:', error.message),
    );
