...
<rect width="686.495" height="686.495" fill="white" transform="translate(26.1875 26.1877)"/>
  ...
  const refreshHomeAppointmentEtaDom = (scope = ROOT) => {
    if (state.currentPage !== "home") return false;
    const etaNode = scope?.querySelector?.("[data-home-appointment-eta]");
    if (!etaNode) return false;
    const appointment = Array.isArray(state.payload?.booking?.activeAppointments)
      ? state.payload.booking.activeAppointments[0] || null
      : null;
    if (!appointment) return false;
    const nextLabel = normalizeText(formatAppointmentEta(appointment.date, appointment.time) || "Скоро");
    if (etaNode.textContent === nextLabel) return true;
    etaNode.textContent = nextLabel;
    return true;
  };
        ...
        <article class="appointment-card${nextAppointment ? " has-booking" : ""}">
          ${nextAppointment ? `
            <div class="section-head appointment-head">
              ...
              <div class="appointment-head-eta" data-home-appointment-eta>${normalizeText(appointmentEtaLabel || "Скоро")}</div>
            </div>
  ...
  const renderProfileVkIdOneTap = async () => {
    ...
    const stack = container?.closest(".profile-vkid-stack");
    const user = state.payload?.user || {};
    const vkAuth = state.payload?.site?.auth || {};
